...
def merge(path, base, test_path, n, dist, out_path):
    ...
    for h in range(img.shape[0]):
        for w in range(img.shape[1]):
            if (img_lab[h][w][0] < 10):
                offset[h][w] = base[level[h][w]] * 0.5  # 0.2 / 0.5 / x / 0.4 / 0.4     ## scale1
            elif (img_lab[h][w][0] > 90):
                offset[h][w] = base[level[h][w]] * 0.1  # 1 / 0.1 / x / 1.2 / 0.2       ## scale2
            else:
                offset[h][w] = base[level[h][w]] * 0.4  # 1 / 0.4 / x / 1.2 / 0.7       ## scale3
...
def main():
    ...
    merge(rt_args.img, base, rt_args.img2, n, rt_args.dist, rt_args.save)
